...
export default function HomeFeedClient({ initialListings }: { initialListings: LFGPost[] }) {
  const { t } = useLanguage();
  const [activeFilter, setActiveFilter] = useState('All');
  const [searchQuery, setSearchQuery] = useState('');

  const filtered = initialListings.filter(post => {
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="px-4 flex flex-col gap-3 pb-4">
        ...
        >
          {/* ... Live banner kodların ... */}
          <div className="relative w-10 h-10 flex-shrink-0">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background: 'rgba(var(--neon-color-rgb), 0.2)' }}>🎮</div>
            <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full status-online" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-orbitron font-bold text-sm" style={{ color: 'var(--neon-color)' }}>{t('home_live_title')}</p>
            <p className="text-xs truncate" style={{ color: 'var(--text-secondary)' }}>{t('home_live_sub')}</p>
          </div>
        </motion.div>
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-12">
            ...
            <p className="font-rajdhani font-semibold" style={{ color: 'var(--text-secondary)' }}>{t('home_no_posts')}</p>
          </div>
        ) : (
          filtered.map((post, i) => (
